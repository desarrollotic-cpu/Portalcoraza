...
@Processor(MOTOR_GLOBAL_QUEUE, { concurrency: 1 })
export class MotorProcessor extends WorkerHost {
  private readonly logger = new Logger(MotorProcessor.name);

  constructor(private readonly monthlyScheduling: MonthlySchedulingService) {
    super();
  }
  ...
  async process(
    ...
    const { tenantId, year, month, tipoCiclo, createMissing, userId } = job.data;
    this.logger.log(
