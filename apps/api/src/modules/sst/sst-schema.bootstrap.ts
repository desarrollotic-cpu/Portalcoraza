...
export class SstSchemaBootstrap implements OnModuleInit {
  ...
  private async ensureGerenciaGrants(): Promise<void> {
    await this.ds.query(`
      ...
      WHERE r.code IN ('GERENCIA', 'ADMIN', 'SUPERADMIN', 'INSPECTOR_SST')
  ...
  /** Sincroniza todos los puestos operativos de Coraza con SST y mantiene el trigger activo. */
  private async ensurePostsSync(): Promise<void> {
    try {
      await this.ds.query(`
        ...
        -- 3. Trigger automático para nuevos puestos
      ...
      `);
      this.log.log('Sincronización de puestos de Coraza y trigger SST listos');
    } catch (err) {
      const msg = err instanceof Error ? err.message : String(err);
      this.log.error(`Error en sincronización de puestos a SST: ${msg}`);
    }
  }
}
