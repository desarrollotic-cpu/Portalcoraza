...
export class MinutaService {
  ...
  private userName(v: VigiaJwtPayload): string {
    return (v.nombre || 'vigilante').trim();
  }
  ...
  async dashboard(v: VigiaJwtPayload) {
  ...
  async historial(v: VigiaJwtPayload, limite = 20, tipo?: string) {
    const lim = Math.min(Math.max(limite, 1), 100);
    const usuario = this.userName(v);
    const chunks: Array<{
      tipo: string;
      fecha: Date;
      id: string;
    ...
    if (want('ENTREGA')) {
      ...
      push(
        ...
        rows.filter(
          (r) =>
            r.vigilanteSaliente.toLowerCase().includes(usuario.toLowerCase()) ||
            r.vigilanteEntrante.toLowerCase().includes(usuario.toLowerCase()) ||
            (r.associateId && r.associateId === v.sub),
        ),
      );
    }
  ...
  async crearVisitante(v: VigiaJwtPayload, dto: MinutaVisitanteDto) {
    const now = this.nowBogota();
    const row = await this.visitantes.save(
      this.visitantes.create({
        ...
        acompana: (dto.acompana || 'No').replace(/^si$/i, 'Si').replace(/^no$/i, 'No'),
  ...
  async crearCorrespondencia(v: VigiaJwtPayload, dto: MinutaCorrespondenciaDto) {
    const now = this.nowBogota();
    ...
    const row = await this.correspondencia.save(
      this.correspondencia.create({
        ...
        vigilanteEntrega: entregado ? this.userName(v) : null,
  ...
  async entregarCorrespondencia(
    ...
    row.vigilanteEntrega = this.userName(v);
  ...
  async crearContratista(v: VigiaJwtPayload, dto: MinutaContratistaDto) {
  ...
  async crearDomiciliario(v: VigiaJwtPayload, dto: MinutaDomiciliarioDto) {
    const now = this.nowBogota();
  ...
  async crearIncidente(v: VigiaJwtPayload, dto: MinutaIncidenteDto) {
    const now = this.nowBogota();
    const row = await this.incidentes.save(
      this.incidentes.create({
        ...
        accionesTomadas: dto.accionesTomadas?.trim() || 'Reportado a supervisión',
  ...
  async crearServicio(v: VigiaJwtPayload, dto: MinutaServicioDto) {
    const now = this.nowBogota();
  ...
  async crearEntrega(v: VigiaJwtPayload, dto: MinutaEntregaDto) {
    const now = this.nowBogota();
  ...
  async registrarSalida(
    ...
    if (tipo === 'VISITANTE') {
      ...
      return { success: true, id, salida: now.toISOString(), message: 'Salida registrada' };
    }
    if (tipo === 'CONTRATISTA') {
